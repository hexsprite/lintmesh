...
const VALID_LINTERS = ['eslint', 'oxlint', 'tsgo', 'biome', 'tsc'] as const;
const VERSION = '0.1.0';

/** Map LinterId from config to LinterName used by adapters */
function linterIdToName(id: LinterId): LinterName {
  // tsc in config maps to tsgo adapter (which falls back to tsc)
  if (id === 'tsc') return 'tsgo';
  return id;
}
...
program
  ...
  .option('--linters <list>', 'Comma-separated linters: eslint,oxlint,tsc,biome', 'eslint,oxlint,tsgo')
  ...
  .action(async (files, opts) => {
    try {
      ...
      const cliLintersProvided = opts.linters !== 'eslint,oxlint,tsgo'; // Check if user provided --linters
